...
function App() {
  ...
  useEffect(() => { // Inicialização
    ...
    if (currentPages.length === 0) {
      ...
      const initialPage = createNewPageObject( INITIAL_PAGE_TITLE, `# ${INITIAL_PAGE_TITLE}\n\nBem-vindo! Use o editor para começar sua história e adicione questões.`, DEFAULT_ACCENT_COLOR, initialPageId );
      ...
      pageToLoadInitially = currentPages.find(p => p.id === effectiveCurrentPageId);
      if (!pageToLoadInitially && currentPages.length > 0) {
        pageToLoadInitially = currentPages[0];
        effectiveCurrentPageId = currentPages[0].id;
      }
    }
    ...
    if (effectiveCurrentPageId) {
        ...
        if (pageToLoadInitially) loadPageDataToInputs(pageToLoadInitially);
        else {
            const firstPage = currentPages.length > 0 ? currentPages[0] : null;
            if (firstPage) {
                setCurrentPageId(firstPage.id);
                loadPageDataToInputs(firstPage);
            } else { // Nenhuma página após carregar do storage e filtrar
                loadPageDataToInputs(null);
                setCurrentPageTitleInput(INITIAL_PAGE_TITLE); // Prepara para nova página inicial
                setCurrentPageId(null); 
            }
        }
    } else { // Nenhuma página e nenhum currentPageId salvo
        loadPageDataToInputs(null);
        setCurrentPageTitleInput(INITIAL_PAGE_TITLE);
    }
    setTestAnswers({});
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => { // Salvar pages
    if (pages && (pages.length > 0 || localStorage.getItem(PAGES_STORAGE_KEY) !== null )) {
      ...
      catch (error) { console.error("Erro ao salvar páginas:", error); }
    } else if (pages && pages.length === 0 && localStorage.getItem(PAGES_STORAGE_KEY)) {
      localStorage.removeItem(PAGES_STORAGE_KEY); // Limpa se ficou vazio após ter algo
    }
  }, [pages]);

  useEffect(() => { // Salvar currentPageId
    if (currentPageId) {
      try { localStorage.setItem(CURRENT_PAGE_ID_STORAGE_KEY, currentPageId); }
      catch (error) { console.error("Erro ao salvar currentPageId:", error); }
    } else { localStorage.removeItem(CURRENT_PAGE_ID_STORAGE_KEY); }
  }, [currentPageId]);

  // --- Manipuladores e Lógica Principal ---
  const handleProcessMarkdown = () => {
    ...
    const isCreatingNewPage = !currentPageId;
    let pageIdToProcess = isCreatingNewPage ? generatePageId(titleTrimmed) : currentPageId;
    const newGeneratedIdFromTitle = generatePageId(titleTrimmed);

    if (!isCreatingNewPage && newGeneratedIdFromTitle !== currentPageId) {
      pageIdToProcess = newGeneratedIdFromTitle;
    }
    
    if (pages.some(p => p.id === pageIdToProcess && p.id !== currentPageId)) {
      alert(`Uma página com o título "${titleTrimmed}" (resultando no ID "${pageIdToProcess}") já existe. Escolha um título diferente.`);
      if (!isCreatingNewPage) {
        const originalPage = pages.find(p => p.id === currentPageId);
    ...
    if (isCreatingNewPage) {
      targetPageData = createNewPageObject( titleTrimmed, markdownInput, currentPageAccentColor, pageIdToProcess );
      currentPages.push(targetPageData);
    } else {
      const existingPageIndex = currentPages.findIndex(p => p.id === currentPageId);
      if (existingPageIndex !== -1) {
        targetPageData = {
          ...
          questions: currentPages[existingPageIndex].questions || [] 
        };
        currentPages[existingPageIndex] = targetPageData;
      } else {
        console.error("Tentando editar página não encontrada:", currentPageId); return;
      }
    }
    ...
    while ((match = linkRegex.exec(contentToScan)) !== null) {
      ...
      const pageExists = currentPages.some(p => p.id === linkedPageId) || newPagesFromLinks.some(p => p.id === linkedPageId);
      if (!pageExists) {
        newPagesFromLinks.push(createNewPageObject(linkedPageTitle, `# ${linkedPageTitle}\n\nEsta página foi criada automaticamente.`, DEFAULT_ACCENT_COLOR));
      }
    }
    
    let finalPages = [...currentPages];
    if (newPagesFromLinks.length > 0) finalPages = [...finalPages, ...newPagesFromLinks];
    
    const uniquePages = finalPages.filter((page, index, self) => index === self.findIndex(t => t.id === page.id))
                                .sort((a, b) => a.title.localeCompare(b.title));
    
    setPages(uniquePages);
    setCurrentPageId(targetPageData.id);
  };

  const handleInternalLinkClick = useCallback((linkedPageTitle) => {
    const linkedPageId = generatePageId(linkedPageTitle);
    const existingPage = pages.find(p => p.id === linkedPageId);
    if (existingPage) loadPage(linkedPageId);
    else {
      const newPage = createNewPageObject( linkedPageTitle, `# ${linkedPageTitle}\n\nEsta página foi criada automaticamente através de um link.`, DEFAULT_ACCENT_COLOR );
      setPages(prevPages => [...prevPages, newPage].sort((a, b) => a.title.localeCompare(b.title)));
      setCurrentPageId(newPage.id);
      loadPageDataToInputs(newPage);
    }
  }, [pages, loadPage, createNewPageObject, loadPageDataToInputs]);
  
  const handleDeletePage = useCallback((pageIdToDelete) => {
    const pageToDelete = pages.find(p => p.id === pageIdToDelete);
    if (!pageToDelete) { console.warn("Tentativa de apagar página não encontrada:", pageIdToDelete); return; }
    if (window.confirm(`Tem certeza que deseja apagar a página "${pageToDelete.title}"?`)) {
      const newPagesArray = pages.filter(page => page.id !== pageIdToDelete);
  ...
  const renderMarkdown = (mdText, pageAccentColorForLinks) => {
    if (typeof mdText !== 'string') return { __html: '' };
    let html = '';
    try { html = marked.parse(mdText); }
    catch (error) { console.error("Erro ao parsear Markdown:", error); html = '<p>Erro ao renderizar conteúdo Markdown.</p>'; }
    html = html.replace(/\[\[(.*?)\]\]/g, (match, pageTitle) => {
      const titleTrimmed = pageTitle.trim();
      if (!titleTrimmed) return match;
      const linkColor = pageAccentColorForLinks || DEFAULT_ACCENT_COLOR;
      const escapedTitle = titleTrimmed.replace(/"/g, '&quot;'); 
      return `<a href="#" class="internal-link" data-link-title="${escapedTitle}" style="color: ${linkColor};">${titleTrimmed}</a>`;
    });
    return { __html: html };
  };

  useEffect(() => { // Listener para links internos
    ...
    setPages(prevPages => prevPages.map(page => {
        if (page.id === currentPageId) {
          ...
          if (questionData.id) updatedQuestions = existingQuestions.map(q => q.id === questionData.id ? { ...q, ...questionData } : q );
            ...
            const newQuestion = { ...questionData, id: uuidv4(), pageId: currentPageId, options: questionData.options.map(opt => ({ ...opt, id: opt.id || uuidv4() })) };
  ...
  const handleResetHistory = useCallback(() => {
    if (window.confirm("Tem certeza que deseja APAGAR TODA a história e começar do zero? Esta ação não pode ser desfeita.")) {
      localStorage.removeItem(PAGES_STORAGE_KEY);
      localStorage.removeItem(CURRENT_PAGE_ID_STORAGE_KEY);
      const initialPageId = generatePageId(INITIAL_PAGE_TITLE);
      const newInitialPage = createNewPageObject( INITIAL_PAGE_TITLE, `# ${INITIAL_PAGE_TITLE}\n\nBem-vindo novamente! A história foi resetada.`, DEFAULT_ACCENT_COLOR, initialPageId );
  ...
  const pageSpecificStyles = currentPageObject ? {
    ...
    '--page-accent-color-link': currentPageObject.accentColor || DEFAULT_ACCENT_COLOR,
    '--page-accent-color-link-hover': currentPageObject.accentColor ? `color-mix(in srgb, ${currentPageObject.accentColor} 80%, black)` : `color-mix(in srgb, ${DEFAULT_ACCENT_COLOR} 80%, black)`,
  } : {
    '--page-accent-color': DEFAULT_ACCENT_COLOR,
    '--page-accent-color-link': DEFAULT_ACCENT_COLOR,
    '--page-accent-color-link-hover': `color-mix(in srgb, ${DEFAULT_ACCENT_COLOR} 80%, black)`,
  };
        ...
        {isEditMode && (
          <>
            <button className="new-page-button sidebar-action-button" onClick={() => {
                ...
                setCurrentPageId(null); setCurrentPageTitleInput(newTitle);
        ...
        <ul>
          {pages.map(page => (
            <li key={page.id} className={page.id === currentPageId ? 'active' : ''}
                style={page.id === currentPageId ? { backgroundColor: page.accentColor || DEFAULT_ACCENT_COLOR, boxShadow: `inset 3px 0 0 color-mix(in srgb, ${page.accentColor || DEFAULT_ACCENT_COLOR} 70%, black)` } : {}}>
              <span onClick={() => loadPage(page.id)} className="page-title-sidebar">{page.title}</span>
              {isEditMode && !(pages.length === 1 && page.id === generatePageId(INITIAL_PAGE_TITLE))}
      ...
      <main className="main-content">
        {isEditMode && (
          <div className="editor-area">
            <input type="text" className="title-input" placeholder="Título da Página" value={currentPageTitleInput} onChange={(e) => setCurrentPageTitleInput(e.target.value)} />
            <textarea className="markdown-input" value={markdownInput} onChange={(e) => setMarkdownInput(e.target.value)} placeholder="Digite seu Markdown aqui... Use [[Nome da Outra Página]] para criar links." />
            ...
            <button onClick={handleProcessMarkdown} className="process-button" disabled={!currentPageTitleInput.trim()}>
              {(currentPageId && pages.some(p => p.id === currentPageId)) ? "Salvar Página" : "Criar Página"}
            </button>
            {currentPageObject && (
        ...
        )} {/* Fim da editor-area condicional */}

        <div id="page-content-display" className={`page-display page-display-wrapper ${!isEditMode && currentPageObject ? 'full-height-display' : ''}`} style={pageSpecificStyles}>
          {currentPageObject ? (
            <>
              ...
              <div dangerouslySetInnerHTML={renderMarkdown(currentPageObject.markdown || '', currentPageObject.accentColor)} />
              {currentPageObject.questions && currentPageObject.questions.length > 0 && (
